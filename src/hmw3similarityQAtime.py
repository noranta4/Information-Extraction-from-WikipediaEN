...
r1 = 'began during the'  # double seed
r2 = 'is active during the'
questions = list(time)
relation = 'time'
simil_threshold = 0.87
print('seed :\t', r1)
...
for item in I:
    ...
    if simil >= simil_threshold:
        if (item[0], item[2]) not in extracted_couples:
            entities = [ent.label_ for ent in nlp(item[0] + ' ' + item[1] + ' ' + item[2]).ents]
            if 'LOC' not in entities and 'GPE' not in entities:
    ...
    if simil >= 0.8:
        if (item[0], item[2]) not in extracted_couples:
            entities = [ent.label_ for ent in nlp(item[0] + ' ' + item[1] + ' ' + item[2]).ents]
            if 'LOC' not in entities and 'GPE' not in entities:  # entity recognition
                extracted_couples.append((item[0], item[2]))
                full_extracted_pentaples.append((item[0], item[2], item[3], item[4], item[5]))
                # print(item[0] + '\t' + 'time' + '\t' + item[2])

f = open('question-answer-pairs.txt', 'w', encoding='utf-8')
        ...
        if (pentaple[0], pentaple1[1]) not in extracted_couples:
            for question in questions:
                if generate_qa(question, pentaple[0], pentaple1[1], 'no') == 0:
                    ...
                    try:
                        if int(pentaple1[1]) > int(pentaple[1]):
